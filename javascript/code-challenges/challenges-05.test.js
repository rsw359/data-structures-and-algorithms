...
const addValues = (arr) => {
  let sum = arr.reduce((plus, value) => plus + value, 0);
...
const addPurchases = (arr) => {
  let total = arr.reduce((total, currentVal) => total + currentVal.purchasePrice, 0);
  return total;
};
...
const returnNames = (arr) => arr.reduce((accumulator, person) => {
...
const reversedString = (str) => {
  let arr = str.split('');
  let rev = arr.reduce((dumb, current) => {
    return current + dumb;
  }, '');
  return rev;
};
