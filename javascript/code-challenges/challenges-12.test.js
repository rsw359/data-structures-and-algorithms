...
const validateWord = (word) => /^\D{5,10}$/.test(word);
...
const hasNumber = (string) => /\D+\d+/.test(string);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 5
...
const validateEmail = (email) => {
  let regularity = /^[a-zA-Z0-9]+\.?[a-zA-Z0-9]+@[a-zA-Z0-9]+\.(net|org|com)\b/;
  return regularity.test(email);
};
